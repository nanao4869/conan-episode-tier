(() => {
  ...
  const SONGS = window.HOLMES || [];
  const GROUPS = window.HOLMES_GROUPS || {};
  const BY_NO = new Map(SONGS.map((s) => [s.no, s]));

  const STORAGE_KEY = "holmesTier.v1";
  const DEFAULT_TITLE = "好きなシャーロック・ホームズの作品 Tier表";
  const DEFAULT_TIERS = [
    { name: "S\n最高", color: "#ff7f7f" },
    { name: "A", color: "#ffbf7f" },
    { name: "B", color: "#ffdf7f" },
  ...
  function itemEl(s) {
    ...
    if (d) return d;
    const grp = GROUPS[s.g] || { label: "", color: "#5a6472" };
    d = document.createElement("div");
    d.className = "item";
    d.dataset.no = s.no;
    d.tabIndex = 0;
    d.setAttribute("role", "button");
    d.title = `${s.title}（${grp.label}）`;

    const art = document.createElement("div");
    art.className = "op-art";
    art.style.backgroundColor = grp.color;
    art.style.color = textColorFor(grp.color);
    const tag = document.createElement("span");
    tag.className = "op-artist";
    tag.textContent = grp.label;
    const title = document.createElement("span");
    title.className = "op-title";
    title.textContent = s.title;
    art.append(tag, title);
  ...
  sortBtn.addEventListener("click", () => {
    ...
    sortBtn.textContent = newestFirst ? "並び：逆順" : "並び：発表順";
    renderPool();
  });
  ...
  // 収録作品集（長編／冒険／思い出…）で絞り込み
  const groupFilterEl = $("groupFilter");
  (() => {
    const count = new Map();
    for (const s of SONGS) count.set(s.g, (count.get(s.g) || 0) + 1);
    for (const [key, g] of Object.entries(GROUPS)) {
      const opt = document.createElement("option");
      opt.value = key;
      opt.textContent = `${g.label}（${count.get(key) || 0}）`;
      groupFilterEl.append(opt);
    }
  })();
  groupFilterEl.addEventListener("change", renderPool);
  ...
  function renderPool() {
    ...
    for (const s of newestFirst ? [...SONGS].reverse() : SONGS) {
      if (placed.has(s.no)) {
        if (group && s.g === group) foundSet.add(s.no);
        continue;
      }
      remaining++;
      if (group && s.g !== group) continue;
      shown++;
    ...
    if (shown === 0) {
      ...
      p.textContent = remaining === 0 ? "すべての作品をTierに入れました！" : "該当する作品がありません";
      frag.append(p);
    }
    poolGrid.replaceChildren(frag);
    for (const [no, el] of itemCache) el.classList.toggle("found", foundSet.has(no));
    poolCount.textContent = group ? `${shown}件表示 ／ 未分類 ${remaining}件` : `未分類 ${remaining} / ${SONGS.length}`;
  }
  ...
  function openDetail(no) {
    ...
    const grp = GROUPS[s.g] || { label: "" };
    $("dFile").textContent = s.en;
    $("dPeriod").textContent = `${s.g === "novel" ? "長編" : `短編集『${grp.label}』収録`}　／　発表：${s.pub}`;
    $("dSummary").textContent = s.summary;
    const links = $("dLinks");
    ...
    const addLink = (href, text) => {
      ...
      a.target = "_blank";
      a.rel = "noopener";
      a.className = "holmes-link";
      a.textContent = text;
      links.append(a);
    };
    addLink(`https://ja.wikipedia.org/wiki/${encodeURIComponent(s.wiki)}`, "Wikipediaで見る");
    if (s.az) addLink(`https://www.aozora.gr.jp/cards/000009/card${s.az[0]}.html`, `青空文庫で読む（訳題：${s.az[1]}）`);
  ...
  $("clearAllBtn").addEventListener("click", () => {
    ...
    if (!confirm("すべての作品を未分類に戻しますか？（行の名前と色は残ります）")) return;
  ...
  const imageName = (format) => `holmes_tier_${dateStamp()}.${format === "jpeg" ? "jpg" : "png"}`;
  ...
  const FORMAT_KEY = "holmesTier.format";
  let saveFormat = "png";
  try {
  ...
  const SITE_TEXT = "シャーロック・ホームズの長編4作・短編56作でTier表が作れるツールです！あらすじやWikipedia・青空文庫へのリンク付き #シャーロックホームズ #ホームズTier表";
  ...
  function exportData() {
    ...
    const a = document.createElement("a");
    a.download = `holmes_tier_${dateStamp()}.json`;
    a.href = url;
    a.click();
